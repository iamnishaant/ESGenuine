  ...
  const exportAnalysis = () => {
    ...
    const payload = {
      ...
      claims: claims.filter(c => c.text.trim()).map(c => ({
        id: c.id, text: c.text, sourceUrl: c.sourceUrl, targetDate: c.targetDate,
      })),
      analysis,
    ...
    a.href = url;
    a.download = 'claim_analysis.json';
    a.click();
    URL.revokeObjectURL(url);
  };

  // No persistence backend exists for user-submitted reports, so "submit" exports
  // the real AI analysis instead of faking a server queue.
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!analysis) {
      toast({
        title: "Analyze first",
        description: "Run analysis before exporting.",
        variant: "destructive"
      });
      return;
    }
    exportAnalysis();
    toast({ title: "Analysis exported", description: "Downloaded as JSON." });
  };
            ...
            <div className="glass-panel p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground text-center">
                  Runs LLM-based claim analysis and cross-claim contradiction detection.
                </p>
              </div>
            </div>
