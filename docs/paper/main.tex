...
We denote by \textbf{S0} the condition in which the taxonomy layer is removed and
each claim retains the model's own free-text aspect. S5 is the shipped
configuration.

\paragraph{What the frozen fixtures contain.} The ablation replays stages over
committed extractor output rather than over a pristine model transcript, and the
distinction matters for how the per-stage results should be read. The stored
records already carry the taxonomy assignment and the repair flags produced by
the revision of the layer that was current when they were written. S0 is
therefore \emph{reconstructed}: we reset the taxonomy assignment, repair flags and
framework tags, then replay the current stack. Two consequences follow. The
reconstruction is conservative for the aggregate result---the S0 baseline retains
repairs we cannot undo, so the reported S1$\to$S5 increments are a lower bound on
what the layer contributes over an unrepaired extractor. It is not conservative
for per-stage attribution of S2 and S3, which the extractor applies before the
fixture is written; we return to this in finding~(ii).
...
The corpus comprises $1{,}730$ extracted claims from six sustainability reports
($461$ PDF pages) published by four companies. One report is a
\textbf{statutory BRSR filing}; the remaining five are \textbf{narrative
investor-relations and ESG reports}, three of which carry a tabular performance
annex. Of the extracted claims, $1{,}394$ ($80.6\,\%$) carry a numeric value, and
the type distribution is $1{,}263$ performance, $370$ narrative and $97$ target.
...
42 pages, 373 raw claims) and a \textbf{narrative investor-relations report}
(91 pages, 247 raw claims).

Two properties of this corpus bound what follows, and we state them here rather
than in the limitations alone. First, \textbf{each disclosure regime is
represented by a single document} in the in-depth evaluation, so the
regime-dependence result of Section~\ref{sec:ablation} is a contrast between two
documents, not between two populations. Second, the two evaluated documents are
the corpus's \emph{most} tractable: the share of claims that the taxonomy leaves
\texttt{uncategorized} is $23.0\,\%$ and $24.7\,\%$ for them, against $52.2\,\%$
and $53.3\,\%$ for the two reports not evaluated in depth. Absolute scores
reported here should therefore be read as an upper region of the corpus range.
...
Stage & BRSR & IR\\
\midrule
S0 no taxonomy layer  & 67.7 & 72.1\\
...
\paragraph{(ii) Two components carry the layer; two others are not identifiable
from this design.} Taxonomy mapping and the rule-based gate account for
essentially all of the gain, so the method is more accurately described as
\emph{taxonomy normalisation plus rule-based repair} than as a five-stage
pipeline. The near-zero deltas for fiscal-year repair and source-value
verification, however, must not be read as evidence that those stages do no work.
Both are applied by the extractor at the time claims are produced, before the
fixtures this ablation replays were frozen; re-applying them to an already-repaired
fixture moves three values on the statutory filing and none on the narrative
report. What the ablation measures for S2 and S3 is therefore the residual between
the revision that produced the fixtures and the current one---not the value of the
operations. We report them as \emph{not identifiable} under this design. Isolating
them would require re-extracting the corpus with each stage disabled at extraction
time, which we did not do.
...
The deterministic layer runs in $0.45$\,ms per claim---$168$\,ms for the 373-claim
statutory filing and $105$\,ms for the 247-claim narrative report, single-threaded,
no network, no API spend (best of 15 runs, commodity laptop CPU). The measurement
covers the layer alone: taxonomy normalisation, key refresh, fiscal-year repair,
source-value verification, gate corrections and the furniture filter. It excludes
the LLM call, which is the point, and PDF parsing, which is upstream and cached.
The absolute figure is hardware-dependent; the claim that matters is the order of
magnitude. Sub-millisecond per claim is free relative to an LLM re-extraction pass,
which is what makes finding (iii) actionable: a deployment can afford to carry
repair rules for every disclosure regime it might encounter, and select among them
per document.
...
\subsubsection*{A stronger extractor, on identical input}

To ask whether the repair layer is merely patching one model's weaknesses, we drove a
second, larger open-weight extractor (\texttt{gpt-oss-120b}) over the \emph{same}
committed page markdown, through the same code path, page set and token budget as the
incumbent (\texttt{llama-3.3-70b}), leaving the model as the only variable. Both arms
are frozen as fixtures. We report recall and grounding restricted to pages
\emph{both} arms produced claims for, so that a page lost to a transport failure
cannot be read as a model finding nothing.

\begin{table}[t]
\centering
\caption{Matched-input model comparison, page-matched. Both arms extract from identical
committed markdown through the same code path.}
\label{tab:models}
\begin{tabular}{llcc}
\toprule
Document & Extractor & Fact recall & Grounding\\
\midrule
BRSR    & \texttt{llama-3.3-70b}  & $41.2\,\%$ & $68.3\,\%$\\
BRSR    & \texttt{gpt-oss-120b}   & $\mathbf{97.2\,\%}$ & $\mathbf{99.1\,\%}$\\
IR      & \texttt{llama-3.3-70b}  & $44.1\,\%$ & $100\,\%$\\
IR      & \texttt{gpt-oss-120b}   & $\mathbf{89.8\,\%}$ & $100\,\%$\\
\bottomrule
\end{tabular}
\end{table}

\textbf{Extractor choice dominates the tabular surface.} On the statutory filing the
stronger model recovers $97.2\,\%$ of enumerated table facts against $41.2\,\%$, and
$89.8\,\%$ against $44.1\,\%$ on the narrative report. The grounding gap is the sharper
result: $31.7\,\%$ of the incumbent's emitted values on the statutory filing do
\emph{not} occur on the page they cite, against $0.9\,\%$ for the stronger model. That
misattribution class is precisely what the layer's source-value check exists to catch,
and it is roughly thirty times rarer in the newer model.

\subsubsection*{What this comparison cannot settle}

We had intended this experiment to answer whether the repair layer's contribution
survives a model upgrade. \textbf{It does not answer that question, and we report the
null rather than an inference.} The reason is structural. Fact recall and grounding read
only a claim's value and page, so they observe the deterministic layer solely where it
\emph{adds, removes or rewrites} a claim. On the table-only path its sole such action---the
furniture filter---removes nothing at either tier, because statutory form furniture is an
artifact of full-document extraction and does not arise here. The gold composite, which is
where the layer's aspect, type and unit corrections would register, cannot be used across
extractors at all: it was sampled from the shipped extractor's own output and matches
$2\,\%$ and $0\,\%$ of the two arms respectively. No lane available to us is both
sensitive to the layer and comparable across models.

One measurement is comparable, and it cuts against the intuition that a better model
needs less repair: the \emph{rate at which the gate intervenes does not fall}. The gate
alters the taxonomy assignment of $15.4\,\%$ of the incumbent's claims and $37.7\,\%$ of
the stronger model's on the statutory filing ($0\,\%$ and $20.9\,\%$ on the narrative
report). An intervention is not evidence of an improvement, so this bounds speculation
rather than settling it---but it is inconsistent with the layer being simply absorbed by
scale, and we decline to claim that it is.

The honest summary is that Section~\ref{sec:ablation}'s deltas are established
\emph{within} one extractor and one document, and this experiment does not extend them
across extractors. What it does establish is that on the tabular surface, the choice of
extractor moves recall and grounding far more than anything measured in
Section~\ref{sec:ablation} moves the composite---and that a deployment choosing between
spending on a better model and spending on deterministic repair has no evidence here that
the second substitutes for the first.

Both arms are single runs at temperature $0.1$; we have no variance estimate, so we treat
gaps of a few points as unresolved and rest only on the large ones.
...
\paragraph{Two documents, and the easier two.} The regime-divergence finding rests
on one document per regime. It supports a claim about these two disclosure
formats, not about ESG reporting in general. The two are also the corpus's most
tractable: $23.0\,\%$ and $24.7\,\%$ of their claims fall outside the taxonomy,
against $52.2\,\%$ and $53.3\,\%$ for the two reports we did not evaluate in
depth. Absolute scores would very likely be lower on the rest of the corpus.

\paragraph{The repair layer is measured against one extractor only.} Section~\ref{sec:ablation}
establishes its contribution within a single model's output. The matched-extractor
experiment does not extend that result, because no available lane is both sensitive to the
layer's corrections and comparable across models---the mechanical lanes cannot see aspect,
type or unit repairs, and the gold composite is tied to the shipped extractor's own
phrasing. We therefore make no claim about whether the layer's value persists, shrinks or
grows with model scale. On the tabular surface specifically, extractor choice moves recall
and grounding by far more than the layer moves the composite.

\paragraph{Reconstructed ablation baseline.} The committed fixtures are extractor
output, not a pristine model transcript: they carry the taxonomy assignment and
repair flags emitted by the layer revision current when they were written. We
reset those fields and replay the current stack, so S0 retains repairs we cannot
undo. The reported layer increments are consequently a lower bound, and the two
stages the extractor applies before writing---fiscal-year repair and source-value
verification---are not separately identifiable, as discussed in finding~(ii).
