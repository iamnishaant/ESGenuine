...
### ⚠️ #21e 🟠 (OPEN) — stored satellite evidence is orphaned by every re-ingest
- **What ran:** checked `satellite_evidence.claim_id` against the (round-3 re-ingested) `claims` table: **0/50 still match.** `ingest_claims_to_db` assigns a fresh `uuid4()` claim_id on every ingest, but `satellite_evidence` keys on it — so each re-ingest breaks the per-claim link. The report-level panel still shows counts (joined by `report_id`, which survives) but they are **stale 2026-07-11 numbers computed on claims that no longer exist**, and 141 rows have `report_id = NULL` (early/test runs).
- **Fix options (not yet done):** (a) re-run `run_satellite_checks.py` on the current corpus (network-heavy: Nominatim 1 req/s + Sentinel-2 STAC ×2/claim); (b) root fix — key evidence on the stable `claim_signature` (metric_key+time+scope) instead of `claim_id`, so it survives re-ingests; and purge the 141 NULL-report rows. Verdict distribution today: 196 inconclusive / 2 supported / 2 not_supported (all microsoft) — mostly inconclusive by honest-first design.
