...
def _print_markdown(res):
    ...
    for key, d in res.items():
        ...
        for name, s in d["systems"].items():
            g = s["gold"]
            print(f"| {name} | {s['claims']} | {_pct(s['recall']).strip()} | "
                  f"{_pct(s['grounding_precision']).strip()} | "
                  f"{g['extraction_score']} | {_pct(g['match_rate']).strip()} |")
        if not d["frontier_available"]:
        ...
        print("\n> Fact recall and grounding precision use mechanical, system-independent "
              ...
              "the gold set was sampled from the shipped extractor's output, so other systems "
              "are scored only where their source sentences coincide (see the match column).")
